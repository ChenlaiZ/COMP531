...
export function linkFaceBook({regularUsername, regularPassword}) {
    return (dispatch) => {
        if(regularUsername && regularPassword){
            resource('POST', 'linkfb', {regularUsername, regularPassword})
            .then((response) => {
                dispatch(updateSuccess(`Link ${regularUsername} success`))
            }).catch((err) => {
                dispatch(updateError(`Link ${regularUsername} error`))
            })
        }
    }
}
...
export function unlinkFaceBook() {
    return (dispatch) => {
        resource('GET', 'unlinkfb').then((response) => {
            dispatch(updateSuccess('Unlink FaceBook success'))
        }).catch((err) => {
            console.log(err)
            dispatch(updateError('Unlink FaceBook error'))
        })
    }
}
/** WEBPACK FOOTER **
