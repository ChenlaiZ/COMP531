...
export function register({username, email, phone, dob, zipcode, password, pwconf}) {
    return (dispatch) => {
        if (!username || !email || !phone || !dob || !zipcode || !password || !pwconf) {
            return dispatch(updateError('All fields must be supplied'))
        }

        const err = validateProfile({username, email, phone, dob, zipcode, password, pwconf})
        if (err.length > 0) {
            return dispatch(updateError(err))
        }

        resource('POST', 'register', {username, email, phone, dob, zipcode, password})
        .then((response) => {
...
export function fbLogin() {
    return (dispatch) => {
        ...
        window.top.location = 'https://stark-ridge-57925.herokuapp.com/login/facebook'
        //dispatch(navToMain())
    }
}
